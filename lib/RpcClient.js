...
function rpc(request, callback) {
  ...
  var req = this.protocol.request(options, function(res) {
    ...
    res.on('end', function() {
      if (res.statusCode == 401) {
        callback(new Error('auroracoin JSON-RPC connection rejected: 401 unauthorized'));
        return;
      }
      if (res.statusCode == 403) {
        callback(new Error('auroracoin JSON-RPC connection rejected: 403 forbidden'));
        return;
      }
  ...
  req.on('error', function(e) {
    var err = new Error('Could not connect to auroracoin via RPC: ' + e.message);
    log.err(err);
    callback(err);
  });
